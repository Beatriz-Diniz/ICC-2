...
void print_most_similar_tracks(struct dados *d, char *tracks, int K){
...
void ler_csv(struct dados *d, int *total_musicas){
   char line[16352], *aux;
   FILE *fp;
   int i = 0, j = 0;
  ler_arquivo(&fp);
...
void countingsort_rec(struct dados *d, int total_musicas) {
  int c[256] = {0};
  int pos[256];

  /*alocar memoria para uma struct copia*/
  struct dados *copy = calloc(total_musicas, sizeof(struct dados));
...
void busca_binaria(struct dados *d, int total_musicas, char *busca, int *r){  
  int inicio = 0;
...
void dissimilaridade(struct dados *d, int q, int k, int total_musicas){
  int i = 0, j = 0;
  char *busca;
  for(i=0; i<q; i++){
    int r =0;
...
int main() {
  ...
  /*Ordenar em ordem alfabetica de id das musicas*/
  qsort(d, total_musicas, sizeof(struct dados), comparar_string);
